# ui/board_ui.py (Bản Fix lỗi ESC)
import json
import pygame
import os
...
class BoardUI:
    ...
    def handle_events(self, event: pygame.event.Event):
        # 1. XỬ LÝ PHÍM ESC ĐẦU TIÊN (ĐỂ THOÁT ĐƯỢC DÙ GAME OVER)
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                return 'QUIT_GAME' # Trả về tín hiệu thoát cho window.py
        ...
        if self.sidebar:
            ...
            if action == 'RESIGN':
                opponent = 'black' if self.game_logic.my_color == 'white' else 'white'
                self.game_logic.winner = opponent
                self.game_logic.game_over = True
        ...
        # NẾU GAME OVER THÌ DỪNG CÁC THAO TÁC KHÁC (NHƯNG ĐÃ CHECK ESC Ở TRÊN RỒI)
                        ...
                        if cmd == "RESIGN":
                            ...
                            if self.confirmation_dialog is None and not self.game_logic.game_over:
                                ...
                                self.confirmation_dialog = UIConfirmationDialog(
                                    ...
                                    window_title="Cầu Hòa",
                                    action_long_desc="Đối thủ muốn cầu hòa. Bạn đồng ý không?",
                                    action_short_name="Đồng ý",
                                    blocking=True
                                )
                                
                        elif cmd == "DRAW_ACCEPT":
                            self.sidebar.add_message("System", "Hai bên đã hòa!")
                            self.game_logic.winner = "draw"
                            self.game_logic.game_over = True
                            if self.confirmation_dialog:
                                self.confirmation_dialog.kill()
    ...
    def draw(self):
        ...
        if self.sidebar:
            ...
            if self.game_logic.my_color:
                ...
                turn_display_text = "Lượt đi: TRẮNG" if self.game_logic.current_turn == 'white' else "Lượt đi: ĐEN"
